...
**Capabilities**
- Writes E2E tests for integration-risk journeys; keeps everything
  provable at a lower level out of the browser
...
- Diagnoses and root-causes flaky tests instead of deleting or retrying
  them into passing
- Configures CI for sharded parallel runs and full failure-artifact
  capture

**Model**: `sonnet` (claude-sonnet-5) — this role writes and edits real
test code and CI config; standard implementation tier, same as
`backend/backend-dev`.
...
infra/runtime constraints outside the suite itself.
