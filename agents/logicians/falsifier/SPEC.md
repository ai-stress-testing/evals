...
- Root-causes a confirmed error — spec ambiguity, missing input, a
  ...
  the quality-management output, not just the "it's wrong"
...
reasoning-bound work as `logicians/logician` and `logicians/code-reviewer`,
paired with the same read-only tool set so the spend buys depth, not
blast radius. Team norm, not a special case.
...
ambiguity rather than agent error. → `testing/reality-checker` for
disproof candidates that require empirical execution to confirm.
